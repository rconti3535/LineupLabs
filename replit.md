...
- **leagues** — id, name, description, type, numberOfTeams, scoringFormat (Roto/H2H Points/H2H Each Category/H2H Most Categories/Season Points), hittingCategories (text array, default R/HR/RBI/SB/AVG), pitchingCategories (text array, default W/SV/K/ERA/WHIP), pointValues (text, JSON string of custom point values for Points leagues — overrides DEFAULT_POINT_VALUES in server/scoring.ts), leagueImage (text, base64 data URL for league photo), isPublic, maxTeams, currentTeams, buyin, prize, status, rosterPositions, maxRosterSize (integer, decouples draft size from scoring positions — Best Ball defaults to 40), draftType, draftDate, secondsPerPick, draftOrder, draftStatus (pending/active/paused/completed), draftPickStartedAt (ISO timestamp for server-side timer), seasonWeeks (integer, default 27), createdBy (FK to users), createdAt
...
7. **Best Ball league type** — Draft-only format; no lineup management, waivers, trades, or roster swaps after draft. Only Roto and Season Points scoring allowed. Optimal lineups are calculated at season end: Season Points uses a single optimal lineup for the entire year; Roto calculates a separate optimal lineup per category. Backend routes block all roster/lineup mutations for Best Ball leagues; UI hides add/drop, swap, waiver, and lineup management controls. Best Ball uses consolidated positions: C, INF (accepts 1B/2B/3B/SS), OF, SP, RP — default roster is ["C","INF","INF","INF","INF","OF","OF","OF","SP","SP","SP","RP","RP"] (13 slots, no bench). Pitchers in the DB are properly classified as SP or RP based on career games started percentage (>=50% starts = SP, <50% = RP) using MLB Stats API data.
